...
require_once 'fields/sauvetage.php'; // Assurez-vous que ce fichier contient le tableau $classes
...
if (isset($_POST['contents']) && is_array($_POST['contents'])) {
    foreach ($classes as $class => $label) {
        if (isset($_POST['contents'][$class])) {
            $stmt = $pdo->prepare("UPDATE sauvetage SET content = :content WHERE class = :class");
...
$contents = [];
$stmt = $pdo->query("SELECT class, content FROM sauvetage");
...
<body>
    ...
    <section>
        <textarea name="contents[inscriptions.h3]" rows="1" cols="80" class="h3"><?= htmlspecialchars($contents['inscription.h3'] ?? '') ?></textarea><br>
        <textarea name="contents[inscriptions.p]" rows="5" cols="80" class="p"><?= htmlspecialchars($contents['inscription.p'] ?? '') ?></textarea><br>
    </section>    
