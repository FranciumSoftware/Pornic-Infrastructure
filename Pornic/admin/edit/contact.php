...
require_once 'fields/contact.php'; // Assurez-vous que ce fichier contient le tableau $classes
...
if (isset($_POST['contents']) && is_array($_POST['contents'])) {
    foreach ($classes as $class => $label) {
        if (isset($_POST['contents'][$class])) {
            $stmt = $pdo->prepare("UPDATE contact SET content = :content WHERE class = :class");
...
$contents = [];
$stmt = $pdo->query("SELECT class, content FROM contact");
...
<body>
    ...
    <section>
        <textarea name="contents[mail.p]" rows="2" cols="80" class="h3" id="edit"><?= htmlspecialchars($contents['mail.p'] ?? '') ?></textarea><br>
        <textarea name="contents[tel.p]" rows="2" cols="80" class="p" id="edit"><?= htmlspecialchars($contents['tel.p'] ?? '') ?></textarea><br>
        <p>Facebook</p>
        <textarea name="contents[facebook]" rows="1" cols="80" class="p"><?= htmlspecialchars($contents['facebook'] ?? '') ?></textarea><br>
        <p>Instagram</p>
        <textarea name="contents[insta]" rows="1" cols="80" class="p"><?= htmlspecialchars($contents['insta'] ?? '') ?></textarea><br>
    </section>
...
<script>
    tinymce.init({
        ...
        menubar: false,
        height: 150, // Assurez-vous de gérer l'upload des images
