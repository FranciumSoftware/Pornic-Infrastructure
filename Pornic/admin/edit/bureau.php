...
require_once 'fields/bureau.php'; // Assurez-vous que ce fichier contient le tableau $classes
...
if (isset($_POST['contents']) && is_array($_POST['contents'])) {
    foreach ($classes as $class => $label) {
        if (isset($_POST['contents'][$class])) {
            $stmt = $pdo->prepare("UPDATE bureau SET content = :content WHERE class = :class");
...
$contents = [];
$stmt = $pdo->query("SELECT class, content FROM bureau");
...
<head>
    <meta charset="UTF-8">
    <title>Édition Bureau</title>
    <link rel="stylesheet" href="../style.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.7.2/css/all.min.css">
</head>
<body>
    <h2>Éditer les contenus de la page Bureau</h2>
     <div id="navbar">
        <a href="./">Accueil</a>
    ...
    <section>
        <textarea name="contents[h3]" rows="1" cols="80" class="h3"><?= htmlspecialchars($contents['h3'] ?? '') ?></textarea><br>
        <textarea name="contents[p]" rows="5" cols="80" class="ul" id="edit"><?= htmlspecialchars($contents['p'] ?? '') ?></textarea><br>
    </section>
