...
function getContentByClass(PDO $pdo, string $class) {
    $stmt = $pdo->prepare("SELECT content FROM jeunes WHERE class = ?");
    $stmt->execute([$class]);
    $row = $stmt->fetch();
...
<html lang="fr">
    <head>
        ...
        <style>
            h2{
                background-image: url('../../ressources/SlideshowJeunes.jpg');
        ...
        <div class="content">
            <div class="body">        
                <h2>Groupe Jeunes</h2>
                    ...
                    <section class="about">
                        <h3><?= getContentByClass($pdo, 'inscriptions.h3') ?></h3>
                        <p><?= getContentByClass($pdo, 'inscriptions.p') ?></p>
                    </section>
            </div>
        </div>
    </body>
