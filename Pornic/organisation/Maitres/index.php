...
function getContentByClass(PDO $pdo, string $class) {
    $stmt = $pdo->prepare("SELECT content FROM maitres WHERE class = ?");
    $stmt->execute([$class]);
    $row = $stmt->fetch();
...
<html lang="fr">
    <head>
        ...
        <style>
            h2{
                background-image: url('../Maitres.jpg');
    ...
    <body>
        ...
        <div class="content">
            <div class="body">        
                <h2>Groupe Maîtres</h2>
                    <section class="about">
                        <h3><?= getContentByClass($pdo, 'about.h3') ?></h3>
                            <ul class="coachs">
                                <li><?= getContentByClass($pdo, 'about.ul') ?></li>
                            </ul>
                    </section>
                    <section class="about">
                        <h3>Horaires</h3>
                    ...
                    <section class="about">
                        <h3>Inscription</h3>
                        <p><?= getContentByClass($pdo, 'incription.p') ?></p>
                    </section>
            </div>
        </div>
    </body>
